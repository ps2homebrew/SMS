...
inline void vert_sub(PbFvec* dst, PbFvec* a, PbFvec* b)
{
	dst->x = a->x-b->x;
	dst->y = a->y-b->y;
	dst->z = a->z-b->z;
...
void move_points(float t, float dt)
{
   ...
   if(move_func_count == 0)
	...
	else if(move_func_count == 1)
	{
		float zionisti = PbCos(t*0.5f);
		float k=0.05f*PbSin(zionisti*M_PI+23);		
		PbFvec FollowMe={ 80*PbCos(t*1.77+50*PbSin(t*-2.5)), 50*PbSin(t*1.5)+150*PbCos(t*-2.34), 120*PbSin(exp(1.0f/PbSqrt(t)) + t*2.5), 1.0f };
		PbFvec FaetterGuf={130*PbCos(t*1.5),50*PbSin(t*1.55),100*PbCos(t*1.1)+1.3*20*PbSin(t*-2.3), 1.0f };

		for(i=0; i<NUM_POINTS; ++i)
		{
			PbFvec nisse0,nisse1,nisse2;
#define h 0.4			
			{
				PbFvec AA;
				vert_sub(&AA, &points[i], &FaetterGuf);
			
				nisse0.x = h * 28 * (AA.y - AA.x);
				nisse0.y = h * (AA.x * (46.92 - AA.z) - AA.y);
				nisse0.z = h * (AA.x * AA.y - 4 * AA.z);
			}
			{
				PbFvec AA;
				vert_sub(&AA, &points[i], &FollowMe);
				nisse1.x = h * 10.0 * (AA.y - AA.x) - nisse0.x;
				nisse1.y = h * (AA.x * (28.0 - AA.z) - AA.y) - nisse0.y;
				nisse1.z = h * (AA.x * AA.y - (8.0 / 3.0) * AA.z) - nisse0.z;
			}				
			{
				PbFvec AA = points[i];
				nisse2.x = k * 9 * (AA.y - AA.x);
				nisse2.y = k * (AA.x * (7 - AA.z) - AA.y);
				nisse2.z = k * (AA.x * AA.y - 19 * AA.z);
			}
#undef h							
			//direction[i] = nisse0*zionisti+nisse1*(1.0f-zionisti)+nisse2*sin(zionisti*M_PI+23);		
			{
				direction[i].x = nisse0.x+nisse1.x*zionisti+nisse2.x;		
				direction[i].y = nisse0.y+nisse1.y*zionisti+nisse2.y;		
				direction[i].z = nisse0.z+nisse1.z*zionisti+nisse2.z;		
			}			
		}
	}
...
float BLUR_RATIO = 27;
#define MAX_INTENS (378.0f / BLUR_RATIO)
...
{
	float last_t, max_t=pInfo->curr_time;
	...
	while(pInfo->frame_count > 0)
	{
	    ...
	    BLUR_RATIO -= dt*25;
	    if(BLUR_RATIO < MIN_BLUR) BLUR_RATIO = MIN_BLUR;
	    
	    move_func_count = (t/max_t)>0.4 ? 1 : 0;
...
#else
			PbMatrixMultiply( &final_matrix, &camera_matrix, &view_screen_matrix );
#endif		    
