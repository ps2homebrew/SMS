...
int fs_dopen  (iop_file_t *fd, const char *name)
{
	...
	mips_memset(fd->privdata, 0, sizeof(D_PRIVATE)); //NB: also implies "file_flag = 0;"
	...
	mips_memcpy(&(((D_PRIVATE*)fd->privdata)->fatdir), &fatdir, sizeof(fatdir));
...
int fs_dread  (iop_file_t *fd, void* data)
{
	...
	do {
		...
		mips_memset(buffer, 0, sizeof(fio_dirent_t));
...
int fs_getstat(iop_file_t *fd, const char *name, void* data)
{
	...
	mips_memset(stat, 0, sizeof(fio_stat_t));
