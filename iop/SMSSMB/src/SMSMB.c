...
static unsigned char s_Win2DOS[ 256 ];
static unsigned char s_DOS2Win[ 256 ];
...
#define FLAG_DOPEN            0x00000001
#define FLAG_DOPEN_1          0x00000002
#define FLAG_LOGIN_PROGRESS   0x00000001
#define FLAG_LOGIN            0x00000002
#define FLAG_STOP             0x00000004
...
static void _smb_name_cvt ( unsigned char* apName, unsigned char* apTbl, int aLen ) {

 unsigned char* lpEnd = apName + aLen;

 for ( ; apName != lpEnd; ++apName ) apName[ 0 ] = apTbl[  apName[ 0 ]  ];
...
static void _smb_file_info_dos2ps2 ( io_dirent_t* apPS2Info, SMBFileInfo* apDOSInfo ) {
 ...
 _smb_name_cvt ( apPS2Info -> name, s_DOS2Win, lLen );
 ...
 *( short* )&lPkt[ 32 ] = apMountCtx -> m_pCtx -> m_UID;
 ...
 lPkt[ 70 ]             = 'D';
 lPkt[ 71 ]             = ' ';
 *( short* )&lPkt[ 72 ] = SMB_ATTR_RDONLY | SMB_ATTR_SYSTEM | SMB_ATTR_HIDDEN | SMB_ATTR_DIR | SMB_ATTR_ARCH;
 *( short* )&lPkt[ 74 ] = 1;
 *( short* )&lPkt[ 76 ] = SMB_FFF_CLOSE_IF_END | SMB_FFF_REQUIRE_RESUME_KEY;
 *( short* )&lPkt[ 78 ] = 1;
 strcpy ( &lPkt[ 84 ], apPath );

 _smb_name_cvt ( &lPkt[ 84 ], s_Win2DOS, lPathLen );
 ...
 *( short* )&lPkt[ 32 ] = apMountCtx -> m_pCtx -> m_UID;
 ...
 _smb_name_cvt ( &lPkt[ 44 ], s_Win2DOS, lNameLen );
...
retry:
 ...
 if ( lSD >= 0 ) while ( 1 ) {
  ...
  if (  !lwip_connect (
          lSD, ( struct sockaddr* )&lAddr, sizeof ( struct sockaddr_in )
         )
  ) {
...
again:
     ...
     if ( lLoginSem < 0 && !lErrCode ) {
      if ( lpSrvCtx -> m_Flags & FLAG_STOP ) return;
      DelayThread ( 2000000 );
      goto retry;
     } else break;
 ...
 if ( lLoginSem >= 0 )
  ...
  while ( 1 ) {
   ...
   CpuSuspendIntr ( &lState );
    ...
    if ( lSD ) {
     lpSrvCtx -> m_Flags &= ~FLAG_LOGIN_PROGRESS;
     lpSrvCtx -> m_Flags |=  FLAG_LOGIN;
     break;
    }  /* end if */
   CpuResumeIntr ( lState );
   DelayThread ( 100 );
...
static int _DrvIOCtl ( iop_io_file_t* apFile, unsigned long aCmd, void* apParam ) {
 ...
 switch ( aCmd ) {
  ...
  case SMB_IOCTL_LOGIN: {
   ...
   for ( i = 0; i < SMB_MAX_SERVERS; ++i )
    if ( !s_ServerCtx[ i ].m_Flags ) break;

   if ( i == SMB_MAX_SERVERS ) return -EMLINK;

   lpSrvCtx = &s_ServerCtx[ i ];
   ...
   lThread.priority  = 64;
   StartThread (
    lpSrvCtx -> m_LoginThreadID = CreateThread ( &lThread ), lpSrvCtx
   );
   ...
   if ( !lpInfo -> m_fAsync ) {
    ...
    if (   (  retVal = lpSrvCtx -> m_Socket >= 0 ? i : -ENOTCONN   ) >= 0  ) lpSrvCtx -> m_Flags = FLAG_LOGIN;
  ...
  case SMB_IOCTL_LOGOUT: {
   ...
   if (   lSrvIdx >= 0                                    &&
          lSrvIdx  < 2                                    &&
          ( s_ServerCtx[ lSrvIdx ].m_Flags & FLAG_LOGIN ) &&
    ...
    TerminateThread ( lSrvIdx = lpSrvCtx -> m_LoginThreadID );
    DeleteThread ( lSrvIdx );
    lpSrvCtx -> m_LoginThreadID = 0;
  ...
  case SMB_IOCTL_MOUNT: {
   ...
   if (   lUnit >= 0                                    &&
          lUnit <  2                                    &&
          ( s_ServerCtx[ lUnit ].m_Flags & FLAG_LOGIN ) &&
          s_ServerCtx[ lUnit ].m_Socket >= 0
   ) {
  ...
  case SMB_IOCTL_ECHO: {
   ...
   retVal = (   lSrvIdx >= 0 && lSrvIdx < 2 && ( s_ServerCtx[ lSrvIdx ].m_Flags & FLAG_LOGIN ) &&
            s_ServerCtx[ lSrvIdx ].m_Socket >= 0 && SMB_Echo ( &s_ServerCtx[ lSrvIdx ] )   ) ? 0 : -ENOTCONN;

  } break;

  case SMB_IOCTL_STOPC: {

   int lSrvIdx = *( int* )apParam;

   if ( lSrvIdx >= 0 && lSrvIdx < 2 ) {

    SMBServerContext* lpSrvCtx = &s_ServerCtx[ lSrvIdx ];

    retVal = 0;

    if ( lpSrvCtx -> m_LoginThreadID ) {

     CpuSuspendIntr ( &lSrvIdx );
      if ( lpSrvCtx -> m_Flags & FLAG_LOGIN_PROGRESS )
       lpSrvCtx -> m_Flags |= FLAG_STOP;
      else if ( lpSrvCtx -> m_Flags & FLAG_LOGIN )
       retVal = -EALREADY;
     CpuResumeIntr ( lSrvIdx );

     if ( !retVal ) {

      iop_thread_info_t lInfo;
     ...
     TerminateThread ( lpSrvCtx -> m_LoginThreadID );
     DeleteThread ( lpSrvCtx -> m_LoginThreadID );
     lpSrvCtx -> m_LoginThreadID = 0;
  ...
  case SMB_IOCTL_SETCP: {

   unsigned char* lpParam = ( unsigned char* )apParam;

   mips_memcpy ( s_DOS2Win + 128, lpParam,       128 );
   mips_memcpy ( s_Win2DOS + 128, lpParam + 128, 128 );
...
int _start ( int argc, char** argv ) {
 ...
 for ( i = 0; i < 256; ++i ) s_Win2DOS[ i ] =
                             s_DOS2Win[ i ] = ( unsigned char )i;
