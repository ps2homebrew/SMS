...
static SMS_INLINE void _mpeg_fill_gmc_buffer ( SMS_MacroBlock* apRefPic, int aMBX, int aMBY, int aSrcX, int aSrcY ) {
 ...
 DMA_RecvS ( DMAC_TO_SPR, g_MPEGCtx.m_pMCBuffer, lpMB, 48 );
 ...
 for ( i = 0; i < 4; ++i ) {
  ...
  DMA_Wait ( DMAC_TO_SPR );
  DMA_RecvS ( DMAC_TO_SPR, g_MPEGCtx.m_pMCBuffer + 2, lpMB + 2, 48 );
...
#else  /* PS2 */
 DMA_SendS(
  DMAC_FROM_SPR, ( uint8_t* )g_MPEGCtx.m_pDest, ( uint8_t* )g_MPEGCtx.m_pMacroBlock[ g_MPEGCtx.m_IdxRes ], 24
 );
...
static SMS_INLINE void _mpeg_fill_mc_buffer_start ( SMS_MacroBlock* apRefPic, int aSrcX, int aSrcY ) {
 ...
 __asm__ __volatile__(
  ...
  ".set reorder\n\t"
  :: "m"( g_MPEGCtx.m_MBY               ),
     "m"( g_MPEGCtx.m_CurPic.m_Linesize ),
     "m"( g_MPEGCtx.m_MBX               ),
     "m"( g_MPEGCtx.m_pCache            ),
     "m"( g_MPEGCtx.m_pMCBuffer         ),
     "m"( g_MPEGCtx.m_fDirtyCache       ),
     "m"( g_MPEGCtx.m_pMCYBuf           ),
     "m"( g_MPEGCtx.m_pMCCbBuf          ),
     "m"( g_MPEGCtx.m_pMCCrBuf          )
 );
#endif  /* WIN32 */
...
#else  /* PS2 */
   DMA_RecvS (  DMAC_TO_SPR, ( uint8_t* )g_MPEGCtx.m_pMacroBlock[ g_MPEGCtx.m_IdxRes ], lpMB, 24 );
   DMA_Wait ( DMAC_TO_SPR );
#endif  /* _WIN32 */
   goto end;
