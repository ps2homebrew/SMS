...
static int _Load_idx1 ( SMS_Container* apCtx, int aSize ) {
...
#ifndef _WIN32
 if ( apCtx -> m_pGUICtx ) apCtx -> m_pGUICtx -> Progress ( "Loading indices...", 0 );
#endif  /* _WIN32 */
 lPerc = lnIdx / 100 + 1;
 ...
 for ( i = 0; i < lnIdx; ++i ) {
  ...
  if (  !( i % lPerc )  ) {
   ...
   if ( apCtx -> m_pGUICtx ) apCtx -> m_pGUICtx -> Progress (  NULL, ( unsigned int )( lPos + 0.5F )  );
...
static int _ReadPacket ( SMS_AVPacket* apPkt ) {
 ...
 while ( lpBuf -> m_CurPos < lpBuf -> m_Size ) {
  ...
  if ( lData[ 0 ] >= '0' && lData[ 0 ] <= '9' &&
   ...
   if ( lpAVIStm -> m_SampleSize ) apPkt -> m_DTS /= lpAVIStm -> m_SampleSize;
