...
static void _draw_text ( char* apStr ) {
 ...
 int lWidth = g_GSCtx.TextWidth ( apStr, 0 );
 int lX     = ( g_GSCtx.m_Width  - lWidth ) / 2;
 int lY     = ( g_GSCtx.m_Height -     26 ) / 2;

 if ( s_Player.m_pIPUCtx ) s_Player.m_pIPUCtx -> Sync ();

 DMA_Wait ( DMA_CHANNEL_VIF1 );

 g_GSCtx.DrawText ( lX, lY, 0, apStr, 0, 0 );

}  /* end _draw_text */

static void _set_colors ( void ) {

 g_GSCtx.SetTextColor ( 0, g_Palette[ g_Config.m_PlayerSCNIdx - 1 ] | 0x80000000 );
 g_GSCtx.SetTextColor ( 1, g_Palette[ g_Config.m_PlayerSCBIdx - 1 ] | 0x80000000 );
 g_GSCtx.SetTextColor ( 2, g_Palette[ g_Config.m_PlayerSCIIdx - 1 ] | 0x80000000 );
 g_GSCtx.SetTextColor ( 3, g_Palette[ g_Config.m_PlayerSCUIdx - 1 ] | 0x80000000 );
...
static void _sms_dummy_video_renderer ( void* apParam ) {
 ...
 u64* lpDMA  = ( u64* )UNCACHED_SEG( s_lDMA );
 int  i      = 0;

 SleepThread ();

 lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 2 ], 1, DMA_REF, 0, ( u32 )s_Player.m_OSDPackets[ 2 ], 0  );
 lpDMA[ i++ ] = 0;

 if ( s_Player.m_pCont -> m_pPlayList ) {

  lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 4 ], 1, DMA_REF, 0, ( u32 )s_Player.m_OSDPackets[ 4 ], 0  );
  lpDMA[ i++ ] = 0;

 }  /* end if */

 lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 3 ], 1, DMA_REF,  0, ( u32 )s_Player.m_OSDPackets[ 3 ], 0  );
 lpDMA[ i++ ] = 0;

 if ( g_Config.m_PlayerFlags & SMS_PF_ANIM ) {

  lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 5 ], 1, DMA_REF, 0, ( u32 )s_Player.m_OSDPackets[ 5 ], 0   );
  lpDMA[ i++ ] = 0;

 }  /* end if */

 lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 1 ] - 1, 1, DMA_REF, 0, ( u32 )( s_Player.m_OSDPackets[ 1 ] + 2 ), 0   );
 lpDMA[ i++ ] = 0;
 lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 0 ] - 1, 1, DMA_REF, 0, ( u32 )( s_Player.m_OSDPackets[ 0 ] + 2 ), 0   );
 lpDMA[ i++ ] = 0;
 lpDMA[ i++ ] = DMA_TAG(  s_Player.m_OSDQWC[ 6 ] - 1, 1, DMA_REFE, 0, ( u32 )( s_Player.m_OSDPackets[ 6 ] + 2 ), 0   );
 lpDMA[ i   ] = 0;
 ...
 while ( 1 ) {
  ...
  if ( s_Flags & SMS_FLAGS_VSCROLL ) {
   ...
   if ( s_Player.m_OSDPLPos == s_Player.m_OSDPLRes ) {
    ...
    lpDMA[ 2 ] = DMA_TAG(  s_Player.m_OSDQWC[ 4 ], 1, DMA_REF, 0, ( u32 )s_Player.m_OSDPackets[ 4 ], 0  );
...
static void _sms_video_renderer ( void* apParam ) {
 ...
 while ( 1 ) {
  ...
  if ( s_Flags & SMS_FLAGS_PAUSE ) {
   ...
   if (  !( s_Flags & SMS_FLAGS_MENU )  ) _draw_text ( "Pause" );
...
static void _sms_audio_only_renderer ( void* apParam ) {
 ...
 while ( 1 ) {
  ...
  if ( lpFrame -> m_PTS == SMS_STPTS_VALUE ) {
   ...
   PlayerControl_UpdateDuration ( s_Player.m_pCont -> m_Duration );
...
static void _sms_play ( void ) {
 ...
 if ( !lfNoVideo ) {
  ...
  if ( s_Player.m_pSubFileCtx ) {
   ...
   s_Player.m_pGUICtx -> Progress ( "Loading subtitles...", 100 );
   ...
   if ( s_Player.m_pSubCtx -> m_ErrorCode ) {
    ...
    sprintf (
     lBuff, "Subtitle: %s error (%d), press X to continue...",
     s_Player.m_pSubCtx -> m_ErrorCode == SubtitleError_Format ?
     "format" : "sequence", s_Player.m_pSubCtx -> m_ErrorLine
    );
...
resume:
   ...
   } else if (  ( lButtons & PAD_START ) && !lfNoVideo  ) {
   ...
   } else if (  lButtons == PAD_RIGHT && ( s_Player.m_pCont -> m_Flags & SMS_CONT_FLAGS_SEEKABLE )  ) {
   ...
   } else if (  lButtons == PAD_LEFT && ( s_Player.m_pCont -> m_Flags & SMS_CONT_FLAGS_SEEKABLE )  ) {
   ...
   } else if ( lButtons == PAD_R1 ) {

    s_Player.m_pIPUCtx -> Pan ( -1 );

   } else if (  lButtons == ( PAD_R1 | PAD_L1 )  ) {

    s_Player.m_pIPUCtx -> Reset ();

   } else if ( lButtons == PAD_CROSS && !lfNoVideo ) {

    PlayerControl_HandleOSD ( 0, 0 );

   } else if ( lButtons == PAD_L2 && !lfNoVideo ) {
   ...
   } else if ( lButtons == PAD_R2 && !lfNoVideo ) {
   ...
   } else if (  lButtons == ( PAD_L2 | PAD_R2 ) && !lfNoVideo  ) {
   ...
   } else if ( lButtons == PAD_CIRCLE && !lfNoVideo ) {
...
SMS_Player* SMS_InitPlayer ( FileContext* apFileCtx, GUIContext* apGUICtx, FileContext* apSubFileCtx, unsigned int aSubFormat ) {
