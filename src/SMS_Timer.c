...
void Timer_Init ( void ) {
 ...
 s_T0HandlerID = AddIntcHandler (   INTC_TIMER0, (  int ( * ) ( int )  )T0_Handler, 0   );
 s_T1HandlerID = AddIntcHandler (   INTC_TIMER1, (  int ( * ) ( int )  )T1_Handler, 0   );

 EnableIntc ( INTC_TIMER0 );
...
void Timer_Destroy ( void ) {
 ...
 DisableIntc ( 9 );
 RemoveIntcHandler ( INTC_TIMER0, s_T0HandlerID );
 RemoveIntcHandler ( INTC_TIMER1, s_T1HandlerID );
...
void Timer_Wait ( unsigned int aPeriod ) {
 ...
 if ( lnParts ) {
  ...
  EnableIntc  ( INTC_TIMER1 );
   for ( i = 0; i < lnParts; ++i ) WaitSema ( s_WaitSema );
  DisableIntc ( INTC_TIMER1 );
