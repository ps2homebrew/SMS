      ...
      case SMS_MKTAG( 'a', 'u', 'd', 's' ): {
       ...
       if ( lStmIdx >= apCtx -> m_nStm )
        ...
        lpStm = apCtx -> m_pStm[ lAudioIdx = lStmIdx ];
                   ...
                   File_GetUShort ( apCtx -> m_pFileCtx );  /* wPriority       */
                   File_GetUShort ( apCtx -> m_pFileCtx );  /* wLanguage       */
        lPreload = File_GetUInt   ( apCtx -> m_pFileCtx );  /* dwInitialFrames */
...
error: retVal = 0;
 if ( retVal ) {
  ...
  _SMS_AVILoadIndex ( apCtx );

  if ( lVideoIdx != -1 && lAudioIdx != -1 && lPreload != 0xFFFFFFFF ) {

   lpStm                   = apCtx -> m_pStm[ lAudioIdx ];
   apCtx -> m_AudioPreload = ( float )lpStm -> m_pIdx[ lPreload ].m_CumLen * ( float )lpStm -> m_Codec.m_BlockAlign / ( float )( lpStm -> m_Codec.m_BitRate >> 3 );

  } else apCtx -> m_AudioPreload = 0.0F;
