...
u32   C68k_Read_Dummy(const u32 adr);
void   C68k_Write_Dummy(const u32 adr, u32 data);
...
s32    C68k_Interrupt_Ack_Dummy(s32 level);
void   C68k_Reset_Dummy(void);
...
s32   C68k_Get_CycleToDo(c68k_struc *cpu)
{
...
s32   C68k_Get_CycleRemaining(c68k_struc *cpu)
{
    if (!(cpu->Status & C68K_RUNNING)) return -1;
...
s32   C68k_Get_CycleDone(c68k_struc *cpu)
{
    if (!(cpu->Status & C68K_RUNNING)) return -1;
...
void   C68k_Release_Cycle(c68k_struc *cpu)
{
    if (cpu->Status & C68K_RUNNING) cpu->CycleIO = cpu->CycleSup = 0;
}

void   C68k_Add_Cycle(c68k_struc *cpu, s32 cycle)
{
