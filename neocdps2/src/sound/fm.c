...
/*		LOG(LOG_INF,("TotalLevel(%3d) = %x\n",t,TL_TABLE[t]));*/
	...
	pom = sinf(2.0 * PI * s / SIN_ENT);	/* sin  -> PS2 = sinf */ 
...
static void FMsave_state_channel(FM_CH *CH,int num_ch)
{
    ...
    for(ch=0;ch<num_ch;ch++,CH++) {
	/* channel */
	create_state_register(ST_YM2610_FM, "op1_out1" ,ch+1, &CH->op1_out[0] , 1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_FM, "op1_out2" ,ch+1, &CH->op1_out[1] , 1*sizeof(int32),REG_INT32);
	...
	for(slot=0;slot<4;slot++) {
	    ...
	    create_state_register(ST_YM2610_FM, "evc"     ,(ch<<4)|slot, &SLOT->evc, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "eve"     ,(ch<<4)|slot, &SLOT->eve, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evs"     ,(ch<<4)|slot, &SLOT->evs, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsa"     ,(ch<<4)|slot, &SLOT->evsa, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsd"     ,(ch<<4)|slot, &SLOT->evsd, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evss"     ,(ch<<4)|slot, &SLOT->evss, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsr"     ,(ch<<4)|slot, &SLOT->evsr, 1*sizeof(int32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "TLL"     ,(ch<<4)|slot, &SLOT->TLL, 1*sizeof(int32),REG_INT32);
...
static void FMsave_state_st(FM_ST *ST)
{
	...
	create_state_register(ST_YM2610_FM, "TIMER A"   ,1, &ST->TA   ,1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_FM, "TIMER Acnt",1, &ST->TAC  ,1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_FM, "TIMER B"   ,1, &ST->TB   ,1*sizeof(uint8),REG_uint8);
	create_state_register(ST_YM2610_FM, "TIMER Bcnt",1, &ST->TBC  ,1*sizeof(int32),REG_INT32);
...
(int) (5.9 / EG_STEP), (int) (11.8 / EG_STEP) };
		CH->pms =
		    (int32) ((1.5 / 1200.0) * pmd_table[v & 7] * PMS_RATE);
		/* b4-5 AMS */
		/* 0 , 1.4 , 5.9 , 11.8(dB) */
...
static void FMsave_state_adpcma(ADPCM_CH *adpcm)
{
	...
	create_state_register(ST_YM2610_ADPCMA, "IL"   ,ch+1, &adpcm->IL , 1*sizeof(int32),REG_INT32);
	...
	create_state_register(ST_YM2610_ADPCMA, "adpcmx"   ,ch+1, &adpcm->adpcmx , 1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcmd"  ,ch+1, &adpcm->adpcmd, 1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcml"   ,ch+1, &adpcm->adpcml , 1*sizeof(int32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "volume"   ,ch+1, &adpcm->volume , 1*sizeof(int32),REG_INT32);
    }
...
/*    int16 *aybuf[3];
    ...
    for (i = 0; i < 0; i++) {
	aybuf[i] = alloca(sizeof(int16) * 16384);
    }
*/
    /* setup DELTA-T unit */
...
//    create_state_register(ST_YM2610, "ym2610"   ,1, F2610   , sizeof(YM2610));
    create_state_register(ST_YM2610, "out_ch"   ,1, out_ch   , 4*sizeof(int32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in1"   ,1, &pg_in1   , 1*sizeof(int32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in2"   ,1, &pg_in2   , 1*sizeof(int32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in3"   ,1, &pg_in3   , 1*sizeof(int32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in4"   ,1, &pg_in4   , 1*sizeof(int32),REG_INT32);
    ...
    /* address register1 */
    create_state_register(ST_YM2610, "address1" ,1, &F2610->address1, 1*sizeof(int32),REG_INT32);
